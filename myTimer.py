...
def printTime(inc):
    ...
    try:
        ...
        #     print(m.stateCode)
        # else:
        #     print(f"{m.stateCode}---exception")
    bmpinfo = screenshot.GetInfo()
    bmpstr = screenshot.GetBitmapBits(True)
    # screenshot.SaveBitmapFile(mem_dc, 'c:\\WINDOWS\\Temp\\screenshot.bmp')
    im_PIL = Image.frombuffer('RGB', (bmpinfo['bmWidth'], bmpinfo['bmHeight']), bmpstr, 'raw', 'BGRX', 0, 1)
    im_PIL.save('c:\\WINDOWS\\Temp\\screenshot.jpg')
    # free our objects
    mem_dc.DeleteDC()
    ...
    newfilename= f"{curr_time.month}-{curr_time.day}-{curr_time.hour}-{curr_time.minute}-{curr_time.second}.jpg"
